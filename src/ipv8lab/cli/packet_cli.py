...
def build_packet(
    ...
    if as_json:
        ...
        table.add_column()
        table.add_row("Source", src_addr.canonical)
        table.add_row("Destination", dst_addr.canonical)
...
def parse_packet(
    ...
    table.add_row("Source", pkt.src.canonical)
    table.add_row("Destination", pkt.dst.canonical)
    table.add_row("Payload length", f"{len(pkt.payload)} bytes")
    table.add_row("Checksum", f"0x{pkt.checksum:08X}")
